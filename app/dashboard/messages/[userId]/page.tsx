...
export default function ChatPage() {
  ...
  // Check if encryption keys were recently regenerated
  useEffect(() => {
    const keyRegenTime = localStorage.getItem('p2p4everything-key-regenerated')
    if (keyRegenTime) {
      const timeSinceRegen = Date.now() - parseInt(keyRegenTime)
      // Show warning for 24 hours after key regeneration
      if (timeSinceRegen < 24 * 60 * 60 * 1000) {
        ...
        const decryptedMessages = await Promise.all(
          storedMessages.map(async (msg) => {
            try {
              ...
              // Check if encryption keys were recently regenerated
              const keyRegenTime = localStorage.getItem('p2p4everything-key-regenerated')
              const wasKeyRegenerated = keyRegenTime && (Date.now() - parseInt(keyRegenTime)) < 24 * 60 * 60 * 1000 // within 24 hours

              return {
                id: msg.messageId,
                message: wasKeyRegenerated
                  ? '[Unable to decrypt - Your encryption keys were recently updated. Messages sent before this cannot be recovered.]'
                  : '[Unable to decrypt - This message may have been encrypted with a different key.]',
                senderId: msg.senderId,
                receiverId: msg.receiverId,
                timestamp: new Date(msg.timestamp),
                senderName: msg.senderId === currentUserDbId
  ...
  return (
    <div className="fixed inset-x-0 top-14 sm:top-16 bottom-0 flex flex-col overflow-hidden bg-gray-50 dark:bg-gray-900 z-40">
      ...
      {showKeyWarning && (
        <div className="flex-shrink-0 border-t border-gray-200 dark:border-gray-700 p-4 bg-orange-50 dark:bg-orange-900/20">
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1">
              <p className="text-sm font-medium text-orange-800 dark:text-orange-200">
                Encryption keys were recently updated
              </p>
              <p className="text-sm text-orange-700 dark:text-orange-300 mt-1">
                Your device&apos;s encryption keys were regenerated. Some older messages may not be decryptable.
            ...
            <button
              onClick={() => {
                ...
                localStorage.removeItem('p2p4everything-key-regenerated')
              }}
